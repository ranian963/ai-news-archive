...
- Comparison-series rule: models released in the same program must not share one role-by-role visual template. Each model gets a distinct cover illustration, accent sequence, story order, and diagram mix based on its differentiator. Reuse a background only when the underlying subject is genuinely the same, and never reuse the same cover illustration within one comparison series.
...
- Weekly cover: the first card keeps the recurring `WEEKLY AI BRIEF → title → two-line scope → date and story count` hierarchy. Its scope copy and date chip use the shared `--weekly-cover-*` thumbnail-readable type tokens; classification tables and editorial methodology stay in the article introduction or detail panel.
...
- Hierarchy: typography, spacing, and hairline dividers do the grouping. Source type badges share one restrained treatment; the panel does not repeat the card artwork's multiple pastel colors. Same-archive links place the source badge on its own row and show the short destination name with `이동`, so long model names do not compete with external-link chrome.
